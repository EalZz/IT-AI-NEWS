...
만약 중요한 기사가 단 하나라도 존재한다면, 그 중요한 기사들만을 골라 각각 핵심을 2~3줄 이내로 직관적으로 요약해주세요. (결과물은 마크다운 포맷과 이모지를 활용해 가독성 좋게 작성하며, 원문 링크를 반드시 포함하세요.)

하지만 만약 모든 기사가 단순한 동향이거나 중요도가 낮다고 판단된다면, 어떠한 부연 설명도 없이 오직 대문자로 "SKIP" 이라고만 답변해 주세요.
\n\n"""
