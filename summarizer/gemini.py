...
def summarize_articles(articles, mode="hourly"):
    ...
    if mode == "daily":
        prompt = """다음은 밤사이 수집된 최신 AI 주요 뉴스들입니다.
독자들을 위해 '오전 8시 정기 뉴스 브리핑'을 작성해주세요. 

조건:
1. 최상단에 `## 🌅 오늘 아침 AI 뉴스 브리핑` 이라는 제목을 달아주세요.
2. 각 기사의 핵심을 2~3줄 이내로 직관적으로 요약해주세요.
3. 가독성이 좋도록 이모지(🌟, 📰 등)를 적절히 활용하세요.
4. 기사별 원문 링크는 반드시 포함되어야 합니다.
...
2. 중요한 기사들만 골라 각각 핵심을 2~3줄 이내로 직관적으로 요약해주세요. (마크다운 포맷과 이모지 활용, 원문 링크 필수)
